...
void SCManager::makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down, double x, double y)
{  
    ...
    locationSeqX.push_back(x);
    locationSeqY.push_back(y);
    //cout<<"push location "<<x<<'&'<<y<<endl; 
    // cout <<polarcontext_vkeys_.size() << endl;
    //write file :
    std::ofstream fout1("/home/sks/testdata/polarcontexts.bin",std::ios::out| std::ios::app | std::ios::binary); //polarcontexts_
    std::ofstream fout2("/home/sks/testdata/polarcontext_invkeys.bin",std::ios::out| std::ios::app | std::ios::binary); 
    std::ofstream fout3("/home/sks/testdata/polarcontext_vkeys.bin", std::ios::out| std::ios::app | std::ios::binary); 
    std::ofstream fout4("/home/sks/testdata/polarcontext_invkeys_mat.bin", std::ios::out| std::ios::app | std::ios::binary); 
    std::ofstream fout5("/home/sks/testdata/locationSeqX.bin", std::ios::out| std::ios::app | std::ios::binary); 
    std::ofstream fout6("/home/sks/testdata/locationSeqY.bin", std::ios::out| std::ios::app | std::ios::binary); 

    fout1<<sc<<std::endl;
    fout2<<ringkey<<std::endl;
    fout3<<sectorkey<<std::endl;
    for (int i=0;i<polarcontext_invkey_vec.size();i++){
        fout4<<polarcontext_invkey_vec[i]<<' ';
    }
    fout4<<std::endl;
    fout5<<x<<std::endl;
    fout6<<y<<std::endl;
    ...
    fout5.close();
    fout6.close();
    //std::cout<<sc.cols<<endl;


} // SCManager::makeAndSaveScancontextAndKeys

void SCManager::makeAndSaveScancontextAndKeys1( pcl::PointCloud<SCPointType> & _scan_down, double x, double y)
{  
    Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1 
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( sc );
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( sc );
...
std::pair<int, float> SCManager::detectLoopClosureID ( void )
{
    ...
    if( min_dist < SC_DIST_THRES )
    {
        ...
        cout << "[test] loop location x : "<<locationSeqX[nn_idx]<<" y : "<< locationSeqY[nn_idx]<<endl;
    }
    else
    {
        std::cout.precision(3); 
...
std::pair<int, float> SCManager::detectLoopClosureID1( void )
{
    int loop_id { -1 }; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")
...
} // SCManager::detectLoopClosureID1
