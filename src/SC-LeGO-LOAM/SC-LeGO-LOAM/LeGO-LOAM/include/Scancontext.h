...
public: 
    ...
    // User-side API
    void makeAndSaveScancontextAndKeys( pcl::PointCloud<SCPointType> & _scan_down, double x, double y);
    void makeAndSaveScancontextAndKeys1( pcl::PointCloud<SCPointType> & _scan_down, double x, double y);
    std::pair<int, float> detectLoopClosureID( void ); // int: nearest node index, float: relative yaw  
    std::pair<int, float> detectLoopClosureID1( void ); 
public:
    // hyper parameters ()
    ...
    std::vector<double> locationSeqX;
    std::vector<double> locationSeqY;
